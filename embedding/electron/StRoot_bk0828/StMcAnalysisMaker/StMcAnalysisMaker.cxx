...
bool StMcAnalysisMaker::InitHists()
{
  ...
  hnHitsvsPt = new TH3F("hnHits","hnHits;nHits;p_{T};#eta",55,0,55,10,0,5,50,-1,1); 
  hnHitsDedxvsPt = new TH3F("hnDedx","hnHitsDedxvsPt;nHitsDedx;p_{T};#eta",55,0,55,10,0,5,50,-1,1); 
  hDCAvsPt = new TH3F("hDCA","hDCA;DCA;p_{T};#eta",100,0,3,10,0,5, 50, -1, 1);
...
int StMcAnalysisMaker::Make()
{
  ...
  //electron 10% embedded into the real events
  mCentrality = getCentralityBin(mpVtx.z(),mEvent->runId(),mMuDst->event()->refMult()*0.9,mCentWeight,refmultcor); 
...
int StMcAnalysisMaker::fillTracks(int& nRTracks, int& nMcTracks)
{
  ...
  for (int ircTrk = 0;ircTrk<mMuDst->globalTracks()->GetEntries();ircTrk++)
  {
    ...
    //only for qa check
    // if (!rcTrack->globalTrack()) cout << "no global track"<<endl;
...
void StMcAnalysisMaker::fillRcTrack( StMuTrack const* const rcTrack,StMcTrack const* const mcTrack)
{
  ...
  hDCAvsPt->Fill(dca,rcTrack->pt(),rcTrack->momentum().pseudoRapidity());  
  // bool goodRcTrack = rcTrack->pt()>McAnaCuts::minPt && fabs(rcTrack->eta())<McAnaCuts::eta;
  // if (!goodRcTrack) return; 
  hnHitsvsPt->Fill(nHitsFit,rcTrack->pt(),rcTrack->momentum().pseudoRapidity()); 
  // hnHitsDedxvsPt->Fill(rcTrack->globalTrack()->nHitsDedx(),rcTrack->pt(),rcTrack->momentum().pseudoRapidity()); 
  hnHitsDedxvsPt->Fill(rcTrack->nHitsDedx(),rcTrack->pt(),rcTrack->momentum().pseudoRapidity()); 
  hRcElectronPtvsCent->Fill(rcTrack->pt(),mCentrality,mCentWeight);
  hMomResolution->Fill(mcTrack->pt(),(rcTrack->pt()-mcTrack->pt())/mcTrack->pt());
}
...
bool StMcAnalysisMaker::passReconstructionCut(StMuTrack const * const pe_1, StMuTrack const * const pe_2, float weight)
{
  ...
  bool passMassCut = mother.m()<0.3; // this is to check if the mass smearing make the mass non-zero
...
bool StMcAnalysisMaker::isGoodTagE(StMuTrack const* const rcTrack) const
{
  ...
  bool goodRcTrack = passnHits && hasFirst3Hit && fabs(dca)<1.5 && rcTrack->pt()>McAnaCuts::minPt && fabs(rcTrack->momentum().pseudoRapidity())<McAnaCuts::eta && fabs(rcTrack->momentum().pseudoRapidity())>0.05;
  return goodRcTrack; 
}
...
bool StMcAnalysisMaker::isGoodPartE(StMuTrack const* const rcTrack) const
{
  ...
  bool passnHits = nHitsFit >McAnaCuts::nHitsFit &&
    ...
    fabs(nHitsdEdx)>McAnaCuts::nHitsdEdx;
  bool goodRcTrack = passnHits && rcTrack->pt()>McAnaCuts::minPt && fabs(rcTrack->eta())<McAnaCuts::partnerEta;
