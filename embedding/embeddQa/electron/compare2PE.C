...
void drawQa(TString head, TPDF* pdf, TCanvas* c)
{
  ...
  {
    TH1* hrc = (TH1*)hDCArc->ProjectionZ("hrc",ip,ip, hDCArc->GetYaxis()->FindBin(-1),hDCArc->GetYaxis()->FindBin(1) );
    hrc->SetDirectory(0);
//  cout <<  hDCArc->GetXaxis()->GetBinLowEdge(ip)<<" " <<  hDCArc->GetXaxis()->GetBinUpEdge(ip)<< endl;
    ...
    TH1* hdata = (TH1*)hDCAdata->ProjectionY("hdata",hDCAdata->GetXaxis()->FindBin(hDCArc->GetXaxis()->GetBinLowEdge(ip)) , hDCAdata->GetXaxis()->FindBin(hDCArc->GetXaxis()->GetBinUpEdge(ip)), hDCAdata->GetZaxis()->FindBin(-1),hDCAdata->GetZaxis()->FindBin(1));
  cout <<"  ...  " <<endl;
