...
#include <iostream>
using namespace std;
void simv2(int npart=1e7,int rdnb=0,TString outname="gammav2")
{
...
void getKinematics(TLorentzVector& b, double const mass)
{
   ...
   float const y = gRandom->Uniform(-2, 2);
...
double getPi0Weight(TLorentzVector* mother,int cent,float EP)
{
   ...
   double phiweight = 1+2*fPi0v2[cent]->Eval(mpt)*std::cos(2*(mphi-EP));
   double ptweight = fpispectra[cent]->Eval(mpt); 
   return ptweight;
}
...
void bookHists()
{
  ...
  {
    fpispectra[ic] = new TF1(Form("pi0spectra_%d",ic),"2*TMath::Pi()*x*[0]*pow(TMath::Exp(-1*[1]*x-[2]*x*x)+x/[3], -[4])",0,30); 
