...
bool StPicoElectronMaker::recenterEventPlane()
{
  ...
  if (DEBUG)cout<<"ok5"<<endl;
    ...
    if(QEta_M.Mod()>0) 
    {
      ...
      for (int i=1;i<21;i++)
      {
        pCosEtaM[mCent]->Fill(i,std::cos(2.*i*EP_M));
        pSinEtaM[mCent]->Fill(i,std::sin(2.*i*EP_M));
      }
    }
    ...
    if (EP_P_Sh>TMath::Pi()) EP_P_Sh-=TMath::Pi();
    if(QEta_P.Mod()>0)
    {
      hEventPlaneCent_P->Fill(mCent, EP_P);
      for (int i=1;i<21;i++)
      {
       pCosEtaP[mCent]->Fill(i,std::cos(2.*i*EP_P));
       pSinEtaP[mCent]->Fill(i,std::sin(2.*i*EP_P));
      }
