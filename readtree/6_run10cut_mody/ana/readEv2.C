void readEv2()
{
  ...
  g5->SetName("HFe200");
  TGraphErrors* g6 = new TGraphErrors(6,x5,y5,0,ysys5);
  g6->SetName("HFe200sys");

  double x7[3],y7[3],yerr7[3],ysys7[3];
  getline(read, tmp,'\n');
  getline(read, tmp,'\n');
  for (int i=0;i<3;i++){
    read>>x7[i]>>y7[i]>>yerr7[i]>>ysys7[i];
  }
  TGraphErrors* g7 = new TGraphErrors(3,x7,y7,0,yerr7);
  g7->SetName("HFe62");
  TGraphErrors* g8 = new TGraphErrors(3,x7,y7,0,ysys7);
  g8->SetName("HFe62sys");

  double x9[3],y9[3],yerr9[3],ysys9[3];
  getline(read, tmp,'\n');
  getline(read, tmp,'\n');
  for (int i=0;i<3;i++){
    read>>x9[i]>>y9[i]>>yerr9[i]>>ysys9[i];
  }
  TGraphErrors* g9 = new TGraphErrors(3,x9,y9,0,yerr9);
  g9->SetName("HFe39");
  TGraphErrors* g10 = new TGraphErrors(3,x9,y9,0,ysys9);
  g10->SetName("HFe39sys");
