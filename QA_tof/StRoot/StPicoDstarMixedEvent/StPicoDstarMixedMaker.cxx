...
void StPicoDstarMixedMaker::initHists(){
  ...
  if (mRunbyRunQA){ 
    ...
    hbadTofMult = new TH1F("hbadTofMult","hbadTofMult",totalNum,0,totalNum);
    hBadTofPos = new TH3F("hBadTofPos","hBadTofPos;x;y;z",50,-220,-170,35 ,50,120,50,-220,-170);
    hBadTofId = new TH3F("hBadTofId","hBadTofId;tray;module;cell",125,-0.5,124.5,33,-0.5,32.5,7,-0.5,6.5);
    hBadTofNum = new TH1F("hBadTofNum","hBadTofNum;x;y;z",totalNum,0,totalNum);
    hgoodTofNum = new TH1F("hgoodTofNum","hBadTofNum;x;y;z",totalNum,0,totalNum);
  }
   //event QA
  ...
  if (mRunbyRunQA && isGoodQaEvent(picoEvent) ){ 
    ...
    for (int itrack=0;itrack<nTracks;itrack++){
      ...
      if (tofmatch) {
        ...
        if (tofQaPion && ptot>0.8 && ptot<1.5) goodtof++; 
          int index2tof = trk->bTofPidTraitsIndex();
          StPicoBTofPidTraits const* const tofPid = mPicoDstMaker->picoDst()->btofPidTraits(index2tof);
          if (1./beta<0.88 && ptot>0.8 && ptot<1.5) {
             StThreeVectorF const btofHitPos = tofPid->btofHitPos();
             hBadTofPos->Fill(btofHitPos.x(),btofHitPos.y(),btofHitPos.z());  
            badtof++;
            int tofid = tofPid->btofCellId();
            hBadTofId->Fill(tofid/192+1,(tofid%192)/6+1,tofid%6+1);
          // cout<<"bad tof beta " <<mRunId<<endl; 
          }
        
        // if (ptot<1&&ptot>0.5&&fabs(trk->nSigmaPion())>4 && tofQaPion) {cout<<"nononono bad pid"<<mRunId<<endl;}
        }
      hBadTofNum->Fill(runnum[mRunId],badtof*1.0);
      hgoodTofNum->Fill(runnum[mRunId],goodtof);
