...
void StPicoElectronMaker::initHists(){
  ...
  hnSigE_pi = new TH2F("hnSigE_pi","pion with tof pid;p(GeV/c);nSigE", 480 ,0,6, 800,-10,10);  //tof pi
  ...
  hphotoVsPt_LS = new TH3F("hphotoVsPt_LS","Mee like sign vs photon pt vs cent;Mee;photon p_{T};cent",120,0,0.4,100,0,10,9,-0.5,8.5);
  hphotoVsPt = new TH3F("hphotoVsPt","Mee like sign vs photon pt vs cent;Mee;photon p_{T};cent",120,0,0.4,80,0,8,9,-0.5,8.5);
  ...
  hnTofMatvsRef->Fill(picoEvent->nBTOFMatch(),picoEvent->refMult());  
   if (strangetof) return kStOK;
    ...
    for (int itrack1=0;itrack1<nTracks;itrack1++){
      ...
      //Fill histograms 
      if (istofpi) hnSigE_pi->Fill(mom.mag(),nSigPE1); 
      if (istofkaon) hnSigE_k->Fill(mom.mag(),nSigPE1); 
      if (istofproton) hnSigE_p->Fill(mom.mag(),nSigPE1);
      if (iselectron) {
