...
namespace anaCuts
{
   ...
   float const Eta = 0.8; 
   float const pTof=0.005;
   float const piTof=0.005;
   float const kTof=0.005;
   
   float const tofYlocal=1.8;
   float EEdcaDaughter=1;  //cm
   float nESigma_partner = 4;
   float nSigmaP_tofsmp =5; // nSigmaCut for tof sample //reduce contamination
   float nSigmaK_tofsmp =5;
   float nSigmaPi_tofsmp =5;
