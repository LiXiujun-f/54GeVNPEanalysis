...
void StPicoElectronMaker::initHists(){
  ...
  hnSigE_e_Dz = new TH2F("hnSigE_e_Dz","Dalitz elecron;p(GeV/c);nSigE", 480 ,0,6,200 ,-5.01,5);  //select PE e
  hnSigE_e_LS = new TH2F("hnSigE_e_ls","photonic elecron like sign;p(GeV/c);nSigE", 480, 0,6, 200,-5.01,5);
  hnSigE_e_LS_Dz = new TH2F("hnSigE_e_ls_Dz","Dalitz elecron like sign;p(GeV/c);nSigE", 480, 0,6, 200,-5.01,5);
      ...
      if (isPE1){
        for (int itrack2=0;itrack2<nTracks;itrack2++){
          ...
          if (passPEtopocut && fabs(mother.rapidity())<1) {
            ...
            if (mother.m()<0.3){
              if (mother.m()<0.004 && unlike) {
                hnSigE_e->Fill(pe_2->gMom().mag(),pe_2->nSigmaElectron());
                if (vtxToV0.perp()<2.5) hnSigE_e_Dz->Fill(pe_2->gMom().mag(),pe_2->nSigmaElectron());
                hinvBeta_e->Fill(pe_2->gMom().mag(),1./beta2);
                hV0->Fill(V0.x(),V0.y(),V0.z());
              }
              if ( mother.m()<0.004 && (!unlike)){
                hnSigE_e_LS->Fill(pe_2->gMom().mag(),pe_2->nSigmaElectron());
                if (vtxToV0.perp()<2.5) hnSigE_e_LS_Dz->Fill(pe_2->gMom().mag(),pe_2->nSigmaElectron());
...
bool StPicoElectronMaker::isBemcElectron(StPicoTrack const * const trk) const 
{
  ...
  if (Id>=0){
    ...
    if (bemctrait){
      ...
      isbemcE = nSMDphi>1&&nSMDeta>1&&btowPhiDz ;
    }
  }
  return isbemcE;
}
