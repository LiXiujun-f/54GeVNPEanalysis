...
// void fitNsigE(int centL=0, int centH=8){
  ...
  for (int j=0;j<bin;j++){
    ...
    // htotsamp->Rebin(5);
    double constL[5]={ 100,1e3,1e2,1e2,1e1};
    // double constL[5]={ 100,1e4,1e4,1e2,1e2};
    double constH[5]={ 5e8,5e9,1e9,5e8,1e7};
    ...
    if (centH==8 && centL==6)
    {
      ...
      // }
      if (pt[j]>0.54&&pt[j]<0.58){
        constL[2]=9.5e7;
        constH[2]=1.5e8;
      }
      // // //K
      if (pt[j]<0.6 && pt[j]>0.4){
        constL[3]=0.5*exp(10.4*pt[j]+7.48);
    ...
    if (centH==5 && centL==2)
    {
      ...
      if (pt[j]<0.58&&pt[j]>0.48) {
        ...
        constH[0]=exp(-5.47*pt[j]+17.3);
      }
      // if (pt[j]<=0.95&&pt[j]>0.7){
      //   constL[0]=exp(-3.45*pt[j]+16.4);
      //   constH[0]=exp(-3.45*pt[j]+16.6);
      // }
      if (pt[j]<1.4&&pt[j]>0.8){
        constL[0]=0.98*exp(0.354*pt[j]*pt[j]+(-4.16)*pt[j]+16);
        constH[0]=1.01*exp(0.354*pt[j]*pt[j]+(-4.16)*pt[j]+16);
      }
      if (pt[j]<4&&pt[j]>2.3){
        constL[0]=0.5*exp(0.354*pt[j]*pt[j]+(-4.16)*pt[j]+16);
        constH[0]=2.5*exp(0.354*pt[j]*pt[j]+(-4.16)*pt[j]+16);
      }

      //p
      // if (pt[j]>0.8 && pt[j]<1){
      //   constL[1]=0.5*exp(4.4*pt[j]+6.89);
      //   constH[1]=100*exp(4.4*pt[j]+6.89);
      // }
      if (pt[j]> 1&& pt[j]<1.5){
        constL[1]=0.1*exp(4.4*pt[j]+6.89);
        constH[1]=100*exp(4.4*pt[j]+6.89);
      }
      //pi
      // if (pt[j]>0.8){
      ...
      if ((pt[j]>1&&pt[j]<1.2) || (pt[j]<1.9 && pt[j]>1.7)){
        constL[2]=exp(-3*pt[j]+20.8);
        constH[2]=exp(-3*pt[j]+21);
      }
      ...
      // if (pt[j]>2.2 && pt[j]<2.4){
      // if (pt[j]>2.2){
      //   constL[2]=0.9*exp(-2.95*pt[j]+20.5);
      //   constH[2]=1.1*exp(-2.95*pt[j]+20.7);
      // }
    ...
    // double lowsigma = meanpt*2.5-2.5;
    // double lowsigma = meanpt*2-2.8;
    // if(meanpt>0.8) lowsigma = -0.5;
    // if(meanpt>1.5) lowsigma = -1;
    // if(meanpt>2) lowsigma = 0;
