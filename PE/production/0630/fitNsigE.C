...
// return;
  ...
  int  bin =i;
  TH1F* hpurity = new TH1F("hpurity", "hpurity;p(GeV);purity",bin,pt);
  ...
  for (int j=0;j<bin;j++){
    ...
    c->cd(1);
    projectionAndFit(he,pt[j],pt[j+1],mean[0],sigma[0],-2,2,-0.6,0.1,0.9,1.1,"e");
    c->cd(2);
    ...
    projAndFitForMg(hpimg,pt[j],pt[j+1],mean[4],sigma[4],0,10,meanpimg,5.2,1.1,1.4,"merged #pi");
    c->cd(6);
    double par[15];
    ...
    // htotsamp->Rebin(5);
    double constL[5]={ 1e4,1e3,1e2,1e1,1e1};
    // double constL[5]={ 100,1e4,1e4,1e2,1e2};
    double constH[5]={ 5e7,5e9,1e9,5e8,1e6};
    // double constH[5]={ 5e6,1e9,1e8,5e7,1e5};
    //e
    //
    if (pt[j]<0.58&&pt[j]>0.55) {
      constL[0]=0.9*exp(-5.31*pt[j]+17.8);
      constH[0]=1.1*exp(-5.31*pt[j]+17.8);
    }
    if (pt[j]<=1&&pt[j]>0.75){
    //   constL[0]=exp(-3.45*pt[j]+16.4);
      constL[0]=0.9*exp(-3.16*pt[j]+16.4);
      constH[0]=1.1*exp(-3.16*pt[j]+16.4);
    ...
    if (pt[j]>2.5){
      constL[0]=1*exp(-2.79*pt[j]+15.8);
      constH[0]=6*exp(-2.79*pt[j]+15.8);
    }
    // //p
    // if (pt[j]>1.7){
    ...
    // }
   if (pt[j]>0.9 && pt[j]<1){
      constL[1]=0.5*exp(5*pt[j]+7.3);
      constH[1]=2*exp(5*pt[j]+7.3);
    }
    ...
    // }
    if (pt[j]>3.5){
      constL[2]=1.*exp(-2.45*pt[j]+20.9);
      constH[2]=20.*exp(-2.45*pt[j]+21.1);
    }
    ...
    // }
    // if (pt[j]>1.1){
    //   constL[3]=1*exp(-3.71*pt[j]+20.6);
    //   constH[3]=2*exp(-3.71*pt[j]+20.6);
    // }
    // //merged pi
    ...
    for (int i=0;i<5;i++){
      ...
      cout<<"set parameter limits" << endl;
    ...
    float eff = fpartical[0]->Integral(lowsigma,2)/fpartical[0]->Integral(-6,6);
 ...
 // gPad->Update();
  ...
  pdf->On();
  pdf->Close();
  TFile* fNsig = new TFile("Nsigma_0_8.root","recreate");
  hpurity->Write();
  hpiratio->Write();
  hkratio->Write();
  hpratio->Write();
