...
// return;
  ...
  int  bin =i;
  TH1F* hpurity = new TH1F("hpurity", "hpurity;electron p(GeV);purity",bin,pt);
  ...
  for (int j=0;j<bin;j++){
    ...
    // htotsamp->Rebin(5);
    double constL[5]={ 1e2,1e3,1e2,1e1,1e1};
    ...
    // }
    // if (pt[j]<1.45&&pt[j]>1){
    //   constL[0]=0.8*exp(-3.16*pt[j]+16.4);
    //   constH[0]=1.1*exp(-3.16*pt[j]+16.4);
    // }
    // if (pt[j]>2.3){
    ...
    // }
   // if (pt[j]>0.7){
   //    constL[1]=0.02*exp(-3.16*pt[j]+16.1);
   //    constH[1]=1*exp(-3.16*pt[j]+16.1);
   // }
    ...
    // }
    // if (pt[j]>1.1){
    //   constL[3]=0.5*exp(-3.71*pt[j]+20.6);
    //   constH[3]=1.5*exp(-3.71*pt[j]+20.6);
    // }
    // //merged pi
 ...
 // gPad->Update();
  ...
  pdf->On();
  pdf->Close();
  TFile* fNsig = new TFile("Nsigma_sys.root","recreate");
  hpurity->Write();
  hpiratio->Write();
  hkratio->Write();
  hpratio->Write();
