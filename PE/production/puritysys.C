...
void puritysys()
{
  ...
  c->Divide(2,1);
  TFile* fmean = new TFile(Form("%s_purity.root",name));
  // TFile* fsys = new TFile(Form("%s_sys.root",name));
  TFile* fsys = fmean;
  TH1F* hmean = (TH1F*)fmean->Get("hpurity_ptdef");
  hmean->SetDirectory(0);
  TH1F* hsys = (TH1F*)fsys->Get("hpurity_ptsys");
  hsys->SetDirectory(0);
  ...
  float x1=1,x2=1.35,x3=0.525,x4=0.575;
  x1 = 0.75; x2 = 1.1;x3 = 0.46;x4 = 0.52; 
  if (name=="Nsigma_2_8") {
     x1 = 0.75; x2 = 1.1;x3 = 0.46;x4 = 0.52; 
  }
  drawBox(x1,-0.1,x2,0.1);
  // drawBox(0.8,-0.1,1.4,0.1);
  drawBox(x3,-0.1,x4,0.1 );
