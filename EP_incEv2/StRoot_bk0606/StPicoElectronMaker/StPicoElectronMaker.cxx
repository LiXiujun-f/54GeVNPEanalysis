...
bool StPicoElectronMaker::recenterEventPlane()
{
  ...
  if(QEta_M.Mod()>0) 
  {
    hEventPlaneCent_M_Re->Fill(mCent, EP_M);
    hEventPlaneCent_M_Sh->Fill(mCent, EP_M_Sh);
    hEventPlaneCent_M->Fill(mCent,Q_M_ori.Phi()*0.5);
  }
...
bool StPicoElectronMaker::getIncElectronv2(float bField,StThreeVectorF pVtx)
{
  ...
  for (int itrack1=0;itrack1<nTracks;itrack1++)
  {
      ...
      if (iselectron) {
        ...
        pIncEv2->Fill(pe_1->pMom().perp(),mCent,cos2deltaPhi,mWeight); 
      }
      
      if (DEBUG) cout << "start loop second pe" <<endl;
      //inclusive charged particle, just for check
      pIncHadronv2->Fill(pe_1->pMom().perp(),mCent,cos2deltaPhi,mWeight);
      hIncHadronv2vsPtvsCent->Fill(deltaPhi,pe_1->pMom().perp(),mCent,mWeight); 

      // pion, just for check
      if (isPion(pe_1,beta) && pe_1->charge()>0) pPionPlusv2->Fill(pe_1->pMom().perp(),mCent,cos2deltaPhi,mWeight);
      if (isPion(pe_1,beta) && pe_1->charge()<0) pPionMinusv2->Fill(pe_1->pMom().perp(),mCent,cos2deltaPhi,mWeight);
          ...
          if (passPEtopocut && fabs(mother.rapidity())<1) {
            ...
            if (mother.m()<0.3){
              if (mother.m()<0.01 && unlike) {
                pTagEv2->Fill(pe_1->pMom().perp(),mCent,cos2deltaPhi,mWeight);
                hPhEv2vsPtvsCent->Fill(deltaPhi,pe_1->pMom().perp(),mCent,mWeight);
                // hV0->Fill(V0.x(),V0.y(),V0.z());
              }
              if ( mother.m()<0.01 && (!unlike)){
                pTagEv2_LS->Fill(pe_1->pMom().perp(),mCent,cos2deltaPhi,mWeight);
