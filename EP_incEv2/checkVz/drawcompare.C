void drawcompare()
{
  TFile* file = TFile::Open("hfv2.root");
  // TProfile* p1 = (TProfile*)file->Get("pEv2");
  TH1F* p1 = (TH1F*)file->Get("hphoE");
  p1->DrawCopy();
  
  file = TFile::Open("hfv2_pico.root");
  // TProfile* p2 = (TProfile*)file->Get("pEv2");
  TH1F* p2 = (TH1F*)file->Get("hphoE");
  p2->SetMarkerColor(kBlue);
  p2->DrawCopy("same");

  // TFile* file = TFile::Open("getshift.out.root");
  // TProfile* pSinEtaP[9];
  // TProfile* pCosEtaM[9];
  // TProfile* pCosEtaP[9];
  // TProfile* pSinEtaM[9];
  ...
  // TFile* file = TFile::Open("getshift.old.root");
