...
void electronv2()
{
  ...
  // TFile* file = new TFile("incE_vz60.root");
  // TFile* file = new TFile("test.full.root");
  // TFile* file = new TFile("incEv2_1.root");
  // TFile* file = new TFile("incEv2_2.root");
  // TFile* file = new TFile("incEv2_3.root");
  ...
  for (int i=1;i<=hphoE->GetNbinsX();i++)
  {
     ...
     cout<<Nls << "  "<< Nul << " " <<v2ls <<" "<<v2ul<<" "<< v2PhE <<endl;
...
//  incE_xj->Draw("psame");
 ...
 l->AddEntry(hphoE,"27GeV photonic electron v2","pe");
 l->AddEntry(pEv2,"27GeV inclusive electron v2","pe");
//  l->AddEntry(incE_xj,"inc. e v2 0-80% from Xiujun","pe");
//  l->AddEntry(pHadv2,"inc. charged hadron v2","pe");
 l->SetHeader("0-60 \%");
 l->Draw();
 TFile* fhf = new TFile("hfv2.root","recreate");
 hphoE->Write();
 pEv2->Write();
