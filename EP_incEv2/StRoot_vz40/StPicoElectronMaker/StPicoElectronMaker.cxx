...
bool StPicoElectronMaker::getIncElectronv2(float bField,StThreeVectorF pVtx)
{
  ...
  for (int itrack1=0;itrack1<nTracks;itrack1++)
  {
      ...
      bool isPE1 = isElectron(pe_1,tofmatch,beta) && fabs(mom.pseudoRapidity())<anaCuts::eEta;
      bool iselectron = isElectron(pe_1,tofmatch,beta) && fabs(mom.pseudoRapidity())<anaCuts::eEta;  //after pid cut, same as first e
      ...
      if (isPE1){
        for (int itrack2=0;itrack2<nTracks;itrack2++){
          ...
          if (fabs(pe_2->gMom().pseudoRapidity())>=anaCuts::eEta) continue;
